...
`tools/check_scales.py` 量过一件事——**机台在画面里变小以后会怎样**（把标注帧缩进灰色画布，
等价于同一裁切里机台变小）：
...
| 30% | 356 | IoU 0.88 ✅ |
| 16% | 178 | IoU 0.84 ✅ |
| 10% | 107 | IoU 0.54 ⚠️ 边缘 |
| 7% | 71 | **IoU 0.03 ❌ 假阳性** |

失败的形态很明确：目标太小时，模型**退回它记住的尺寸先验**——半径恒在 27% 左右、
位置随便，分数只有 **0.11~0.14**。

所以策略是：

1. **模型优先**：分数 ≥ **0.15** 就用它（所有实测假阳性都在 0.15 以下，刚好被滤掉）
2. **经典检测器兜底**：模型没出框、或分数不够 → 走经典圆检测器
3. 两者在 50 张上的成绩：**模型 49/50、经典 49/50、配合策略 49/50**（同一张勉强帧）

> 为什么不是"两个都同意才用"：实测那样反而掉到 48/50 —— 有一张模型 IoU 0.95 完全正确、
> 经典 0.19 错得离谱，而"分歧时信经典"把它坑了。一张样本撑不起统计，
> 所以按原理定：模型存在的理由就是泛化到经典规则没覆盖的机况。

**现场含义**：手机大致对准机台就行，别指望它从很远处把机台捞出来 —— 屏在画面里
太小的时候谁都救不了。先凑近，再按「对准机台」。
