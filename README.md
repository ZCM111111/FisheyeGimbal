...
3. Adjust `Lens half FOV` and `Circle scale` until the image circle is fully covered without excessive black edges.
4. Select `Equidistant` or `Equisolid`, then adjust `Radial k1` and `Radial k2` until straight lines are straight.
5. Adjust `Center X` and `Center Y` if the lens is off-center.
6. Press `Recenter`, then move the phone and check the locked direction.
