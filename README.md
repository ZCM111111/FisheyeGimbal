...
| **手动标定** | 面板里带一个直线参考网格，对着门框、桌沿这类长直线拖滑条，边看边调 |
...
1. 夹上附加镜，**之后不要挪动它的位置**
2. 打开右上角「**校正**」面板，展开「**镜头标定**」
3. 点「**打开参考网格**」——屏幕上会出现直的参考线
4. 对着**有长直线的场景**（门框、桌沿、显示器边框），调「**径向 K1**」把弯曲的直线掰直
5. 直线偏向一侧时调「**中心 X / Y**」；看面板里的「**覆盖**」，90~100% 是理想区间
6. 切到「锁定」模式，故意把手机歪着，按「重新回中」——画面应该立刻转正
7. 点「**录像**」开始录，再点停止 → 自动存进相册
